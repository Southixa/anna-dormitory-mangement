<template>
    <div class="w-full mt-4">
        ...
        <div class="w-full flex justify-center items-center">
            <p class="text-xl font-medium text-gray-600">ລາຍງານແຈ້ງການທັງໝົດ</p>
        </div>
            ...
            <table class="border border-gray-300 max-w-[1200px] text-[10px]">
                <thead>
                    <tr>
                        ...
                        <th class="border border-gray-300 px-[8px]">ວັນທີສ້າງ</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="loading">
                    ...
                    <tr v-for="(item, index) in filterAnnouncement" :key  ="index">
                        <td class="border border-gray-300 px-[8px]">{{ index + 1 }}</td>
                        <td class="border border-gray-300 px-[8px]">
                            <div class="w-[20px] h-[20px]">
                                <img class="w-full h-full object-cover" :src="getUrl(item?.announcement_thumbnail, 100)" />
                            </div>
                        </td>
                        <td class="border border-gray-300 px-[8px]">{{ item?.announcement_title }}</td>
                        <td class="border border-gray-300 px-[8px]">{{ item?.announcement_detail }}</td>
                        <td class="border border-gray-300 px-[8px]">{{ timeFormatter(item?.created_at) }}</td>
                    </tr>

                    <tr v-if="!loading && filterAnnouncement.length == 0">
                        <td colspan="100%" >
...
const getAllAnnouncement = async () => {
    ...
    filterAnnouncement.value = addIndex;
}
...
const filterByDate = () => {
    if(!range.value){
        filterAnnouncement.value = announcements.value;
        return;
    }
    filterAnnouncement.value = announcements.value.filter((item) => {
        return (new Date(item?.created_at) >= range.value[0]) && (new Date(item?.created_at) <= range.value[1])
    })
    console.log("filterByDate =>", filterAnnouncement.value);
}

const exportExcel = () => {
  if(filterAnnouncement.value.length === 0){
    message.error("ບໍ່ມີຂໍ້ມູນໃຫ້ Export")
    return;
  }
  const worksheet = XLSX.utils.json_to_sheet(filterAnnouncement.value)
