<template>
    <div class="w-full mt-4">
        ...
        <div class="w-full flex justify-center items-center">
            <p class="text-xl font-medium text-gray-600">ລາຍງານການເຂົ້າເຊົ່າທັງໝົດ</p>
        </div>
        <div class="w-full grid grid-cols-12 mt-4">
            <div class="col-start-2 col-span-4">
                <p>ເລືອກໄລຍະເວລາເຂົ້າເຊົ່າ</p>
            ...
            <table class="border border-gray-300 max-w-[1200px] text-[10px]">
                <thead>
                    <tr>
                        ...
                        <th class="border border-gray-300 px-[8px]">ວັນທີເຂົ້າເຊົ່າ</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="loading">
                    ...
                    <tr v-for="(item, index) in filterCustomers" :key  ="index">
                        ...
                        <td class="border border-gray-300 px-[8px] text-yellow-500 ">ກຳລັງຈອງ</td>
...
const getAllCustomers = async () => {
    ...
    addIndex = addIndex.filter((item) => item?.renting_status == "booking");
...
const filterByDate = () => {
    ...
    filterCustomers.value = customers.value.filter((item) => {
        return (new Date(item?.renting_start_date) >= range.value[0]) && (new Date(item?.renting_start_date) <= range.value[1])
    })
